...
##remove conditions I didn't do in the scope and fix strain names:
DataComparisonCFU<-PA14DataStats %>% filter (condition == "Static") %>% select(bug, strain, CFUmean, logCFUmean, condition, time)%>% as.data.frame() %>% mutate(Strain = recode(DataComparisonCFU$strain, wt = "PA14wt", mut = "PA14mut"))
...
#Now we go to Sa
SaLinearModel <- lm(MeanLogTotals ~logCFUs, data = comparisonDf %>% filter (Species == "Sa"))
coefficients(SaLinearModel)
get_regression_summaries(SaLinearModel, print = TRUE)

SaLinearModel1 <- lm(MeanLogTotals ~logCFUs, data = comparisonDf %>% filter (Species == "Sa")%>% filter(!time == 0))
coefficients(SaLinearModel1)
get_regression_summaries(SaLinearModel1, print = TRUE)
...
ggplot()+
  ...
  scale_color_gradient(low="blue", high="red")+
  annotate("text", x = 7.9, y = 4.75, label = expression(paste("y=-2.44x+25.73  ", R^2, "= 0.619")), color = "red", size = 8)+
