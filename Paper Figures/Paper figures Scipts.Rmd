  ...
  if(Strain == "mut"){etiqueta = bquote(paste(italic('P. aeruginosa '*Delta*"pqsL")))}
    ...
    staticWtPlot<-ggplot()+
      ...
      theme(plot.margin = unit(c(.5,.5,0,1), "cm"),
            axis.title.x = element_text(margin = margin(0,0,0,0),size=13 ),axis.text.x = element_text(margin = margin(1,0,2,0),size=11),#this is  for the x-axis text: "Time (hours)"
    ...
    plot_grid(shakingWtPlot+theme(legend.position=c(.6,.4)),staticWtPlot+theme(legend.position=c(.6,.4)),
                      align='v',labels = c("A", "B"),nrow = 1,label_size= 20, label_x = .02, label_y = 1)
    #plot_grid(prow,legend,rel_widths = c())
    #legend positions for wt are .7,.6, for mutant is .6, .4
    }
#Graph logs
...
#data disk difussion
#####
#This is the data for normalizing the data with the yields
paYields<-read.csv(file='/Users/jupabago/Documents/Whiteley/PROJECTS/pqsL SCFM2/Pa growth yields 16 hrs/Pa growth yields 16 hrs.csv',header=T)
paYieldsSummary<-paYields %>% group_by(condition, strain, bio.rep) %>% summarise(CFUs = mean(bugs)) %>% ungroup()

paYieldsSummaryFinal<-paYieldsSummary%>% group_by(condition, strain) %>% summarise(CFU = mean(CFUs)) %>%
  mutate(CondStrain = case_when(
    condition == "shaking" & strain == "pqsL" ~ "pqsLShake",
    condition == "shaking" & strain == "wt"~ "wtShake",
    condition == "static" & strain == "pqsL"~ "pqsLStat",
    condition == "static" & strain == "wt"~ "wtStat")) %>% ungroup()%>% 
  mutate(logCFU = log10(CFU)) %>% mutate(normalization = CFU/CFU[CondStrain=="wtShake"])#normalized it to wt shaking
...
#to normalize this I am going to have to change the way this works, diameter is the value measure and the disk is 6 in diameter
diskData <- diskData %>% mutate(radius1 = value1/2,radius2 = value2/2,meanRadii = (val1+val2)/2 ) %>% mutate(area = pi*(meanRadii^2))#this removes the diameter of the disks
diskData1<-diskData %>% mutate(
...
#normalized counts
normInhibition<-merge(paYieldsSummaryFinal, diskData1, by= "CondStrain") %>% mutate(normInhibit = area/normalization)
diskDataStats<-normInhibition %>% group_by(strain.x, condition.x,CondStrain, bio.rep) %>% summarise(meanTech = mean(area)) %>% ungroup() %>%group_by(strain.x, condition.x) %>% summarise(meanBio = mean(meanTech), SDBio = std.error(meanTech))
diskDataStats$strain <- factor(diskDataStats$strain.x, levels = c("wt", "pqsl"))
write.csv(normInhibition, "~/Documents/Whiteley/PROJECTS/PaSaProject/Staph killing assays/normalized disk difussion assays.csv")
#####
#t-tests disk difussion
#####
diskWtShaking<-normInhibition %>% filter(strain.x == "wt" & condition.x == "shaking")
diskMutShaking<-normInhibition %>% filter(strain.x == "pqsL" & condition.x == "shaking")
diskWtStatic<-normInhibition %>% filter(strain.x == "wt" & condition.x == "static")
diskMutStatic<-normInhibition %>% filter(strain.x == "pqsL" & condition.x == "static")

#there is no rep 1 for mutant strain, so remove first one from wt to make paired t-test
diskWtStatic1<-normInhibition %>% filter(strain.x == "wt" & condition.x == "static") %>% filter(!bio.rep==1)
diskWtShaking1<-normInhibition %>% filter(strain.x == "wt" & condition.x == "shaking")%>% filter(!bio.rep==1)

#t-tests
diskWtTtest<-t.test(x = diskWtShaking$area, y = diskWtStatic$area, paired = T)
diskMutTtest<-t.test(x = diskMutShaking$area, y = diskMutStatic$area, paired = T)
diskShakingTtest<-t.test(x = diskWtShaking1$area, y = diskMutShaking$area, paired = T)
diskStaticTtest<-t.test(x = diskMutStatic$area, y = diskWtStatic1$area, paired = T)
diskWtStaticMutShakingTtest<-t.test(x = diskMutShaking$area, y = diskWtStatic1$area, paired = T)
diskMutStaticWtShakingTtest<-t.test(x = diskWtShaking1$area, y = diskMutStatic$area, paired = T)
...
#####
GraphStaphKilling<-function(Strain){ 
figure1C<-ggplot()+
      geom_errorbar(data=diskDataStats %>% filter (strain.x == Strain), aes(x = condition.x, ymin = meanBio - SDBio, ymax = meanBio + SDBio),width = 0)+
      geom_point(data=diskDataStats %>% filter (strain.x == Strain), aes(x=condition.x, y=meanBio), size=3)+
      scale_x_discrete(name =expression(paste(italic('P. aeruginosa'), ' culture')), breaks = c("static","shaking"), labels =c("Static","Mixed"))+
    ylim(0,450)+
  labs( y = bquote('Zone of inhibition ('*mm^2*")"), shape = "") +
...
figure1CD<-GraphStaphKilling("wt")
figure2CD<-GraphStaphKilling("pqsL")
#####
ggplot()+
  geom_errorbar(data=diskDataStats %>% filter (strain.x == "wt"), aes(x = condition.x, ymin = meanBio - SDBio, ymax = meanBio + SDBio),width = 0)+
  geom_point(data=diskDataStats %>% filter (strain.x == "wt"), aes(x=condition.x, y=meanBio), size=3)
...
hist.plank.Agg.Stats<-hist.plank.Agg.Sumary %>% group_by(Species,Condition, ConditionNew,Strain,Aggregation) %>% 
  mutate(MeanFreq = mean(FrequencyAggregation), SDFreq = sd(FrequencyAggregation), MeanBiomass = mean(BiomassAggregation), SDBiomass = sd(BiomassAggregation),MeanDensity = mean(DensityAggregation), SDDensity = sd(DensityAggregation) )
...
ggplot()+
  ...
  #geom_point(data = hist.plank.Agg.Stats %>% filter(Strain == strain),aes(x=Aggregation,y=BiomassAggregation*100, group = ConditionNew),           shape = 1,size=2,position = position_dodge( width = .9),show.legend = FALSE)+
  geom_errorbar(data = hist.plank.Agg.Stats%>% filter(Strain == strain),aes(x=Aggregation,ymin=(MeanBiomass-SDBiomass)*100, ymax = (MeanBiomass+SDBiomass)*100, group = ConditionNew),width = 0, size=.5,position = position_dodge(width = .9))+
...
mutate(MeanFrequencyLogBin = mean(FrequencyLogBin),SDFrequencyLogBin = sd(FrequencyLogBin),
          MeanDensityLogBin = mean(DensityLogBin), SDDensityLogBin = sd(DensityLogBin))
...
#rows below add 
new.rowWt.frequency <- data.frame(Species = 'Sa', Condition = "Pa:Sa 1:1",ConditionNew = "Co",Strain = "PA14wt", LogBin =paste0("(100, ",bquote("\U221E"), ")"), MeanDensityLogBin = 0,SDDensityLogBin = 0)
new.rowMut.frequency <- data.frame(Species = 'Sa', Condition = "Pa:Sa 1:1",ConditionNew = "Co",Strain = "PA14mut", LogBin =paste0("(100, ",bquote("\U221E"), ")"), MeanDensityLogBin = 0,SDDensityLogBin = 0)
...
PlotFreqLogBinProp<-function(strain){
  ...
  ggplot()+
    ...
    #geom_jitter(data = df, aes(x=LogBin,y=DensityLogBin*100, group = ConditionNew),shape = 1,size=2,position = position_dodge( width = .9))+
    geom_errorbar(data = df,aes(x=LogBin,ymin=(MeanDensityLogBin-SDDensityLogBin)*100, ymax = (MeanDensityLogBin+SDDensityLogBin)*100, group = ConditionNew),width = 0, size= .5,position = position_dodge( width = .9))+
    facet_grid(.~Species)+
...
top_Row1<-plot_grid(figure1AB,figure1CD,align='vh',nrow = 1, rel_widths = c(2.5,1))
plot_grid(top_Row1,figure1EF, align='v',nrow = 2, rel_heights = c(1,1.5))
ggsave2("/Users/jupabago/Documents/Whiteley/PROJECTS/PaSaProject/Spatial paper Data/GrowthCurves/Figure 2.tiff",
        plot = ggplot2::last_plot(), scale = 1,width = 12,height = 7.7,units = "in", dpi = 1200,limitsize = TRUE)

top_Row2<-plot_grid(figure2AB,figure2CD,align='vh',nrow = 1, rel_widths = c(2.5,1))
...
figure4 <- plot_grid(MixedAggregateFrequencyPlot,sa.saPlot,sa.paPlot,align='vh',labels = c("A","B", "C"),hjust = -1,nrow = 1)
figure4
```
```{r biomass per bin}
