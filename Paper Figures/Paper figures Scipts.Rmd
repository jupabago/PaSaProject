...
plot_grid(plotsFigure1cd,legendFigure1C,rel_widths = c(3, .6))
}   
...
sa.paPlot<-
  ggplot()+
  geom_point(data = enrichmentDataFinal %>% filter(Relation == "Sa->Pa"),aes(y=meanEnrichment, x = Strain),size=4, shape = 1)+
  ...
  labs(y = expression(atop("Enrichment distance ",paste(italic("S. aureus") %->% italic("P. aeruginosa "),"("*mu*"m)"))), x = "" ) +
  ...
  scale_x_discrete(name ="", breaks = c("wt","mut"), 
                   labels = c(bquote(paste(italic("P. aeruginosa"), " wt")), bquote(paste(italic(Delta*"pqsL")))))+
  ylim(1, 9)+
  theme_cowplot()+
  theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
sa.saPlot<-
  ggplot()+
  geom_point(data = enrichmentDataFinal %>% filter(Relation == "Sa->Sa"),aes(y=meanEnrichment, x = Strain),size=4, shape = 1)+
...
figure7 <- plot_grid(MixedAggregateBiomassPlot,sa.saPlot,sa.paPlot,align='vh',labels = c("A","B", "C"),hjust = -1,nrow = 1)
figure7
```
```{r biomass per bin}
...
tobCFUPlot<-
  ...
  labs(y = expression(paste(italic("S. aureus "), "CFU (",Log[10]),")"), x = "", fill = "") +
  scale_fill_grey()+
  theme_cowplot()+
  theme_half_open()+
  background_grid()
