...
hist.plank.Agg.Stats<-hist.plank.Agg.Sumary %>% group_by(Species,Condition, ConditionNew,Strain,Aggregation) %>%
  summarise(MeanFreq = mean(FrequencyAggregation), SEFreq = std.error(FrequencyAggregation), MeanBiomass = mean(BiomassAggregation), SEBiomass = std.error(BiomassAggregation))
...
```
```{r Aggregate histograms logarithmic bins}
...
PlotFreqLogBin<-function(strain, species,titulo){
  ...
  ggplot()+
    ...
    geom_label(data = df %>% filter(Sample == "rep1"),aes(x=as.factor(LogBin),y=1, label =round(BiomassLogBin*100,digits=1), color = "rep1"),size=4)+
...
PlotFreqLogBin("PA14wt", "Sa", "Sa WT")
PlotFreqLogBin("PA14wt", "Pa", "Pa WT")
PlotFreqLogBin("PA14mut", "Sa", "Sa pqsL")
PlotFreqLogBin("PA14mut", "Pa", "Pa pqsL")
