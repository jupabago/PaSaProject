...
hist.plank.Agg<-hist.5 %>%  group_by(Species, Sample,Condition,Strain)%>% 
  mutate(Aggregation = case_when(max == 5 ~ "Planktonic",
                        max>5 ~ "Aggregate")) %>% ungroup() %>% 
  group_by(Species,Sample,Condition,Strain,Aggregation) %>% mutate(FrequencyAggregation = sum(frequency), BiomassAggregation = sum(BiomassBin), DensityAggregation = sum(Density))
hist.plank.Agg.Stats<-hist.plank.Agg %>% group_by(Species,Condition, ConditionNew,Strain,Aggregation) %>%
...
hist.5.LogBinSummarize<-hist.5 %>%  group_by(Sample,Species,Condition,Strain)%>% 
  mutate(LogBin = case_when(max <= 10 ~ 10,
                        max<= 100    ~ 100,
                        max<= 1000   ~ 1000,
                        max<= 10000  ~ 10000,
...
#Now separate them by strain, since I am doing that anyways
hist.LogBin.wt <-hist.5.LogBin %>% filter(Strain == "PA14wt")
hist.LogBin.mut<-hist.5.LogBin %>% filter(Strain == "PA14mut")
