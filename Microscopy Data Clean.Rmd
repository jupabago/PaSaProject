...
#only biomass
mixedAggDataDualSlim<-mixedAggDataDual %>% group_by(Sample, Strain) %>% summarise (AggregatesMixed = n(), PaBiomassMixed = sum(Pa), SaBiomassMixed = sum(Sa), TotalBiomassMixed = sum(Size))

#Compare all objects to dual-species objects
combinedMixedAggData<- bind_cols(mixedAggDataAllSlim, mixedAggDataDualSlim)
combinedMixedAggData<-combinedMixedAggData %>% mutate(PercentAggs = AggregatesMixed/Aggregates, PercentPaBiomass =  PaBiomassMixed/PaBiomass*100, PercentSaBiomass =  SaBiomassMixed/SaBiomass*100, PercentTotalBiomass = TotalBiomassMixed/TotalBiomass*100) %>% group_by(Strain) %>% mutate(MeanAggs = mean (PercentAggs), MeanPaBiomass = mean(PercentPaBiomass), MeanSaBiomass = mean(PercentSaBiomass), MeanTotalBiomass = mean(PercentTotalBiomass), SETotalBiomass = std.error(PercentTotalBiomass))

combinedMixedAggData$Strain <- factor(combinedMixedAggData$Strain, levels = c("PA14wt", "PA14mut"))
### I think this is the final figure
MixedAggregatePlot<-ggplot()+
...
tTestMixWt<-combinedMixedAggData %>% filter(Strain == "PA14wt") %>% pull(PercentTotalBiomass)
tTestMixMut<-combinedMixedAggData %>% filter(Strain == "PA14mut") %>% pull(PercentTotalBiomass)
tTestMixAggs<-t.test(x = tTestMixWt, y = tTestMixMut, paired = F)
