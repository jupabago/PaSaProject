...
allDataHistograms.5df <- allDataHistograms.5df %>% filter(TimePoint== 5) %>% filter( Condition %in% c("Sa Mono", "Pa Mono", "Pa:Sa 1:1")) %>% mutate(ConditionNew = recode(Condition, "Sa Mono" = "Mono", "Pa Mono" = "Mono", "Pa:Sa 1:1" = "Co"))
...
mono<- allDataHistograms.5df %>% filter(ConditionNew == "Mono") %>%  group_by(max, Species, ConditionNew, Strain) %>% summarise(Frequency = sum(frequency))
co<- allDataHistograms.5df %>% filter(ConditionNew == "Co") %>% group_by(max, Species, ConditionNew, Strain) %>% summarise(Frequency = sum(frequency))

#this is still showing the bins per sample, so I want to add the bins on each sample
ggplot()+
  geom_point(data = both %>% filter(Species == "Sa"), aes(y = max,x = CombinedFrequency, color = ConditionNew),stat = "identity", width = .6)+
  scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
                              #coord_flip() +  # Flip axes
                              theme(plot.title = element_text(hjust = .5), 
                                    axis.ticks = element_blank()) +   # Centre plot title
                              theme_cowplot()+
    theme(plot.title = element_text(hjust = 0.5))  # Color palette

ggplot()+
  geom_point(data = both %>% filter(Species == "Sa"& ConditionNew=="Mono"), 
                 aes(x = max, y= -1000-CombinedFrequency,color = ConditionNew), size = 2, alpha = 0.8)+
  geom_point(data = both %>% filter(Species == "Sa"& ConditionNew=="Co"), 
                 aes(x = max, y=1000+CombinedFrequency, color = ConditionNew), size = 2, alpha = 0.8)+
  #geom_label(aes(x = max, y = 0, label = max), 
#         inherit.aes = F,
...
monoPlot<-ggplot()+
  geom_point(data = mono %>% filter(Species == "Sa"),aes(x = max, y= Frequency), size = 2, alpha = 0.8)+
  scale_y_continuous(trans=reverselog_trans(base=10),
                     labels=trans_format("log10", scales::math_format(10^.x)))+
  scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)),limits = c(1e0,5e5), position = "top")+
  coord_flip()+
  xlab("")+
  ylab("Frequency")+
  theme_cowplot()+
...
coPlot<-ggplot()+
  geom_point(data = co %>% filter(Species == "Sa"),aes(x = max, y= Frequency), size = 2, alpha = 0.8)+
  scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)),limits = c(1e0,5e5))+
  coord_flip()+
  xlab("")+
  ylab("Frequency")+
  theme_cowplot()+
...
plotHist3DBiomassCountsLog(allDataHistograms.5df %>% filter(Species == "Sa"), "Sa co-culture vs mono")
