...
#Now make it tidy and do some stats:
cleanResults<-ResultsData %>% group_by(TimePoint,Sample,Strain,Condition,Species) %>% summarise(Totals = sum (Value))%>% mutate(LogTotals = ifelse(Totals>0, log10(Totals), 0))#this adds up all the data on each slice and then gets the log of that total
...
GetResults<-function(sample, samplename){
  folder<- paste0('/Users/jupabago/Documents/Whiteley/PROJECTS/Spatial paper Data/Microscopy Data/Other Data1/',sample,'/results')#the path needs to be "Other Data1"
  fileList<-list.files(path = folder, full.names = TRUE)
  growthDf<-data.frame()
  for (position in 0:5){
    currentPosition<-fileList[grep(paste0("p",position,"."),fileList)]
...
GraphGrowthCurvesExpCleanCoCulture(cleanResultsStats)
```
#This 
```{r new threshold images aggregates data}
GetAggregates<-function(sample, samplename){
...
allAggregates<-allAggregates %>% filter(!Size<0.5)

ggplot()+
  geom_point(data = allAggregates)
  scale_y_log10(limits= c(1e4,1e7), name=expression(paste('Biomass from voxel counts' )), breaks = c(10^(seq(5,10,.5))), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  labs(title =expression(paste(italic("S. aureus"), " growth in mono in SCFM2" )),  x = "Time (hours)",shape = "Culture condition", color = "Antiobiotic/Control") 
```

```{r new threshold images aggregates graph}
TotalHistogram<-function(df){
  ...
  histogram<-histogram[!(histogram$Counts==0),]
  histogram%>%mutate(BiomassBin=Counts*Midpoint)->histogram
  histogram%>%mutate(NormBiomassBin=BiomassBin/totalbiomass)->histogram
  return(histogram)
}

testSplit<-split(allAggregates, c("Sa","Pa"))
allAggregates1<-allAggregates %>% filter(Species %in% c("Pa", "Sa"))
allAggregates$Strain<-as.factor(allData$Strain)
allAggregates$Sample<-as.factor(allData$Sample)
allAggregates$TimePoint<-as.factor(allData$TimePoint)
allAggregates$Position<-as.factor(allData$Position)

allData<-allAggregates %>% group_by(Species, Position, TimePoint, Sample, Strain)
yas<- group_split(allData)

ggplot()+
  geom_point(data = allAggregates, aes(x = TimePoint, y= Position, color = Sample, shape = Species))+
  facet_wrap(.~Strain)
...
plotHist3DBiomassCounts<-function(df1, title){
  ggplot()+
    geom_point(data=df1,aes(x=Midpoint, y=Counts, color = source))+
    scale_y_log10(name=expression(paste('counts' )), breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    xlab('Biomass (um^3)')+
    ylab('Counts')+
    ggtitle(paste0("Raw biomass Distribution (bin size = 10 um) ",title))
    #facet_wrap(~TimePoint, nrow=4)
  }
plotHist3DBiomassBinBiomass<-function(df1, title){
  ggplot()+
    geom_point(data=df1,aes(x=Midpoint, y=BiomassBin, color = source))+
    scale_y_log10(name=expression(paste('counts' )), breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    xlab('Biomass (um^3)')+
    ylab('Counts')+
...
plotHist3DBiomassNormBinBiomass<-function(df1, title){
  ggplot()+
    geom_point(data=df1,aes(x=Midpoint, y=NormBiomassBin))+
    #scale_y_log10(name=expression(paste('counts' )), breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    xlab('Biomass (um^3)')+
    ylab('Counts')+
