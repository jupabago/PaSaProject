...
```{r threshold Sa by time (not useful)}
...
redSubtractedDataSummary<-redSubtractedData[[1]]%>% filter(Condition == "Pa:Sa 1:1"& Strain %in% c("PA14wt", "PA14mut"))
...
nonSubtractedDataSummary<-nonSubtractedData[[1]]%>% filter(Condition == "Pa:Sa 1:1"& Strain %in% c("PA14wt", "PA14mut"))
paNonSubtractResults<-nonSubtractedData[[1]]%>% filter(Species == "Pa") 
saNonSubtractResults<-nonSubtractedData[[1]]%>% filter(Species == "Sa") 

#comparing the data from red 
ggplot()+
...
saRedSubtractResults1<-saRedSubtractResults %>%ungroup() %>% group_by(Sample, TimePoint) %>% 
  ...
  group_by(Strain) %>% mutate (repDifference1.2 = (abs(Totals[Sample == "rep1"] -Totals[Sample == "rep2"]))/Totals ) %>% 
  mutate (repDifference2.3 = (abs(Totals[Sample == "rep2"] -Totals[Sample == "rep3"]))/Totals ) %>% 
  mutate (repDifference1.3 = (abs(Totals[Sample == "rep1"] -Totals[Sample == "rep3"]))/Totals ) %>% ungroup()
...
ggplot()+#plotting the absolute difference between replicates and strains
  geom_jitter(data = tidysaRedSubtractResults1 %>% filter(!Comparison=="mutantRatio"),
              aes(x=TimePoint,y=Difference, color = Comparison),size=3, height = 0, width = .2)+
