...
tTestData<-list(diskWtTtest, diskMutTtest,diskShakingTtest, diskStaticTtest, diskWtStaticMutShakingTtest)
TtestTable<-name <- function(dataPoint) {
  data <-data.frame(dataPoint$data.name, dataPoint$p.value)
}
tTestDataTable<-bind_rows(map(tTestData, TtestTable))
```
...
#this is the original Figure but it needs to be split in two parts
    DiskPlot<-ggplot()+
      geom_errorbar(data=diskDataStats1, aes(x = strain, ymin = meanBio - SDBio, ymax = meanBio + SDBio, group = condition),position = position_dodge(width = .6), width = .2)+
      geom_point(data=diskDataStats1, aes(x=strain, y=meanBio, shape =CondStrain),position = position_dodge2(width = .6), size=3)+
      scale_shape_manual(values = c(1,2,16,17),limits=c("wtStat","pqsLStat","wtShake", "pqsLShake"), 
                     labels = c(bquote(paste(italic('P. aeruginosa'), " wild-type static")),
                                bquote(paste(italic('P. aeruginosa '), Delta*"pqsL static")),
    ...
    legendFigure2 <- get_legend(luminescencePlot + theme(legend.box.margin = margin(0, 0, 0, -10)))
    figure2 <- plot_grid(DiskPlot+theme(legend.position="none"),luminescencePlot+theme(legend.position="none"),
...
#this figure needs to be split to show only wt data and mutant data and limit the time point to 7 minutes for the luminescence.
    ...
    DiskPlotWt<-ggplot()+
      ...
      scale_shape_manual(values = c(1,2),limits=c("static","shaking"), 
                     labels = c(bquote(paste(italic('P. aeruginosa'), " wild-type static")),
                                bquote(paste(italic('P. aeruginosa'), " wild-type stirred"))))+
      scale_x_discrete(name ="", breaks = c("static","shaking"), labels =c("Static","Stirred"))+
    ylim(0,17.5)+
  labs( y = "Zone of inhibition (mm)", shape = "Growth condtion") +
      theme_cowplot(12)
    ...
    luminescencePlotMut<-
      ggplot()+
      geom_line(data=luminescenceData1 %>% filter (Strain == "pqsL") %>% filter (TimePoint<8), aes(x=TimePoint, y=Mean, group = Condition), size=.5, show.legend = FALSE)+
      geom_point(data=luminescenceData1%>% filter (Strain == "pqsL") %>% filter (TimePoint<8), aes(x=TimePoint, y=Mean, shape = Condition), size=3)+
      scale_y_log10(name=expression(paste(italic('S. aureus'), " % luminescence vs control" )),limits = c(1,121), labels = c(1,10,100),breaks = c(1, 10, 100))+
      labs( x = "Time (minutes)",shape = "") +
      scale_shape_manual(values = c(16,17),limits=c("static", "shaking"), 
                     labels = c(bquote(paste(italic('P. aeruginosa '), Delta*"pqsL static")),
